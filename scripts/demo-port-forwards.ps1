...
Write-Host ""
Write-Host "=== CircleGuard demo - port-forwards up ===" -ForegroundColor Green
Write-Host ""
foreach ($f in $forwards) {
    Write-Host ("  {0,-26} http://127.0.0.1:{1}" -f $f.label, $f.local)
}
Write-Host ""
Write-Host "  Grafana dashboard: 'CircleGuard - Overview' (login admin/admin)" -ForegroundColor Cyan
Write-Host "  Gate demo (GREEN/RED): ./scripts/demo-qr-token.ps1 -Call" -ForegroundColor Cyan
Write-Host ""
